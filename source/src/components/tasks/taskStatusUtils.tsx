...
function serializeFooterRelevantTask(task: TaskState): string {
  const parts = [task.id, task.type, task.status]
  ...
  switch (task.type) {
    case 'local_bash':
      ...
      break
    case 'local_agent':
      parts.push(task.agentType, String(task.isBackgrounded))
      break
    case 'remote_agent':
      parts.push(
        String(task.isUltraplan),
        task.ultraplanPhase ?? '',
        String(task.isBackgrounded),
      )
      break
    case 'remote_autowork':
      parts.push(task.mode, task.host ?? '', task.entryPoint, task.action)
      break
    case 'in_process_teammate':
      parts.push(
        ...
        String(task.isIdle),
        String(task.awaitingPlanApproval),
        String(task.shutdownRequested),
      )
      break
    case 'local_workflow':
      ...
      break
    case 'monitor_mcp':
      parts.push(task.serverName ?? '')
      break
    case 'dream':
      parts.push(task.dreamType ?? '')
      break
  }
...
/**
 * Stable primitive selector value for footer/task-pill subscribers.
 * Includes only fields that affect background-task/footer rendering so
 * outputOffset/log/message churn does not repaint the footer forever.
 */
export function getBackgroundTaskRenderSignature(tasks: {
  [taskId: string]: TaskState
}): string {
  const entries: string[] = []
  for (const task of Object.values(tasks) as TaskState[]) {
    if (!isBackgroundTask(task) && !isPanelAgentTask(task)) {
      continue
    }
    entries.push(serializeFooterRelevantTask(task))
  }
  entries.sort()
  return entries.join('|')
}
