...
export const AUTO_MODE_DESCRIPTION = "Auto mode lets CT handle permission prompts automatically — CT checks each tool call for risky actions and prompt injection before executing. Actions CT identifies as safe are executed, while actions CT identifies as risky are blocked and CT may try a different approach. Ideal for long-running tasks. Sessions are slightly more expensive. CT can make mistakes that allow harmful commands to run, it's recommended to only use in isolated environments. Shift+Tab to change mode.";
type Props = {
  onAccept(): void;
  onDecline(): void;
