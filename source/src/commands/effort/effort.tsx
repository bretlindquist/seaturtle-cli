...
function formatEffortLabel(value: EffortValue): string {
...
function normalizeEffortInput(value: string): string {
  const normalized = value.toLowerCase().trim();
  if (['xhigh', 'x-high', 'x_high', 'extra high', 'extra-high', 'extra_high'].includes(normalized)) {
    return 'max';
  }
  return normalized;
}
...
export function showCurrentEffort(appStateEffort: EffortValue | undefined, model: string): EffortCommandResult {
  ...
  if (shouldUseOpenAiCodexProvider() || shouldUseGeminiProvider()) {
    ...
    if (effectiveValue === undefined) {
      return {
        message: `Reasoning level: auto (currently ${displayedLevel})`
      };
    }
    const description = getEffortValueDescription(getDisplayedEffortLevel(model, appStateEffort));
    return {
      message: `Current reasoning level: ${displayedLevel} (${description})`
    };
  }
  if (effectiveValue === undefined) {
...
export function executeEffort(args: string, model: string): EffortCommandResult {
  const normalized = normalizeEffortInput(args);
...
function ExecuteEffortAndClose(t0) {
  const {
    args,
    onDone
  } = t0;
