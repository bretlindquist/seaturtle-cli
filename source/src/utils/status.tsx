...
export async function buildInstructionProperties(): Promise<Property[]> {
  const files = await getMemoryFiles();
  const preferredProjectFile = files.find(file => file.type === 'project' && basename(file.path) === 'AGENTS.md') ?? files.find(file => file.type === 'project' && ['SEATURTLE.md', 'CLAUDE.md'].includes(basename(file.path)));
  if (!preferredProjectFile) {
    return [];
  }
  const instructionFileName = basename(preferredProjectFile.path);
  return [{
...
export function getStatusModelDisplayLabel(mainLoopModel: string, effortValue: EffortValue | undefined): string {
  const modelLabel = getModelDisplayLabel(mainLoopModel);
  if (!modelSupportsEffort(mainLoopModel)) {
    return modelLabel;
  }
  return `${modelLabel} (reasoning ${getDisplayedEffortLevel(mainLoopModel, effortValue)})`;
}
export function getStatusPermissionLabel(permissionMode: PermissionMode): string {
  if (permissionMode === 'bypassPermissions') {
    return 'Full Access';
  }
  return permissionModeTitle(permissionMode);
}
