...
export function getComputerUsePrompt(): string {
  return `
- Runs a provider-routed computer-use loop against the user's real local desktop
...
Usage notes:
  ...
  - This works only when the active runtime supports provider-routed computer use and local computer-use support is enabled
`
}
