...
export function getSwordsOfChaosRelevantMemory(
  ...
  return {
    ...
    activeThread: memory.activeThread,
    liveThreadTitle: memory.liveThreadTitle,
    canonThreadTitle: memory.canonThreadTitle,
    storyChapter: memory.storyChapter,
    chapterTitle: memory.chapterTitle,
    storyTension: memory.storyTension,
    currentObjective: memory.currentObjective,
    carryForward: memory.carryForward,
    continuation: memory.continuation,
    sceneState: memory.sceneState,
    characterDevelopment: memory.characterDevelopment,
    lastOutcomeKey: memory.lastOutcomeKey,
    threadOmen: memory.threadOmen,
    magicState: memory.magicState,
    seaturtleGlimpsed: save.seaturtle.appearances > 0,
    seaturtleOpeningPending: save.worldFlags.includes(
      'seaturtle:pending-opening-glimpse',
    ),
